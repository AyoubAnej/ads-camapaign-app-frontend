...
export const EditCampaignModal: React.FC<EditCampaignModalProps> = ({
  ...
  const [startDate, setStartDate] = useState(campaign.campaignStartDate ? campaign.campaignStartDate.split('T')[0] : '');
  const [endDate, setEndDate] = useState(campaign.campaignEndDate ? campaign.campaignEndDate.split('T')[0] : '');
  const [globalState, setGlobalState] = useState<GlobalState>(campaign.globalState);
  ...
  useEffect(() => {
    if (campaign) {
      ...
      setStartDate(campaign.campaignStartDate ? campaign.campaignStartDate.split('T')[0] : '');
      setEndDate(campaign.campaignEndDate ? campaign.campaignEndDate.split('T')[0] : '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to update campaign. Please try again.');
    } finally {
